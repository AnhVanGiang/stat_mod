...
drop1(model)
```
Now we need to check the significance of our new model: looking at the ANOVA table, all terms turn out to be significant. The same thing happens when looking at the $p$-values of the Summary table.
...
```
The following lines show the expected number of awards a student enroled program $1$ and with a grade of $55$ in mathematics:
...
Below, we see the values for the Pearson's Chi Squared statistic $P$ and deviance $D$. A priori, we can say they are not very off. To be more. rigourous, however, we can compute for $\phi = P / (n-p-1)$, which should be below $1$ and. not very off from $1$. Similarly, the obtained deviance $D$ divided by the deegrees of freedom of the residual gives a value that is very close to $1$.  
