...
From the plot below, it is of course not clear that the type of program a student follows has an effect on the amount of awards he or she can get. Therefore, we will have to perform some tests.
...
```
We decide to convert the variable program into a factor. Afterwards we perform a test without taking the variable 'math' into account. The results for the ANOVA –with the Chi Squared test– yield a significant $p$-value for the factor 'prog'. Afterwards, we can take a look at the summary of the model: it says that programs $1$ and $2$ are significant enough, whereas program number $3$ does not appear to have such a relevant effect. Moreover, looking at the absolute value. of the coefficients, we can rank their relevance, from high to low, in the following order: program $2$, $1$ and $3$.
...
Now we are taking into account the variable 'math' in our model. When performing the first test, we notice that the interaction term between the factor 'prog' and the variable 'math' does not turn out to be relevant. Checking the AIC values for the models we note that indeed the model that excludes interaction term seems to work better, for it displays a lower value for the AIC. Therefore, we decide to drop that term.
...
drop1(model)
```
Now we need to check the significance of our new model: looking at the ANOVA table, all terms turn out to be significant enough. The same thing happens when looking at the $p$-values of the Summary table.
...
To infer what's the best program choice for a student that is only interested in obtaining awards, we can add a column containing the fitted values of the model to our original data. Afterwards, we can filter the rows by program and compute the average expected amount of awards. By doing that, we reinforce our hypothesis of program $2$ being the best one, followed by progran $1$ and program $3$.
...
```
The following lines show the predicted number of awards a new student enroled program $1$ and with a grade of $55$ in mathematics:
...
Below, we see the values for the Pearson's Chi Squared statistic $P$ and deviance $D$. A priori, we can say they are not very off. To be more rigourous, however, we can compute for $\phi = P / (n-p-1)$, which should be below $1$ and not very far from $1$. Similarly, the obtained deviance $D$ divided by the deegrees of freedom of the residual gives a value that is very close to $1$.  
...
Here, we can check for overdispersion. Note that our $\phi$ is below $1$, which is a indicates that we probably do not have overdispersion. In addition, the deviance $D$ is very close to $1$, which is coherent with the hypothesis of not having overdispersion.
